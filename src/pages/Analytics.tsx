...
export default function Analytics() {
  ...
  // Fetch leads for analytics (replacing opportunities)
  ...
  // Fetch flight metrics
  const { data: flightMetrics = [] } = useQuery({
    queryKey: ['flight-metrics'],
    queryFn: async () => {
      const { data, error } = await supabase
        .from('flight_metrics')
        .select('*')
  ...
  // Lead stats (replacing opportunity stats)
  ...
  // Lead status distribution (replacing opportunity status)
  ...
  // Active leads count (no value tracking)
  const activeLeadsCount = leads
    .filter(l => !['converted', 'lost'].includes(l.status || '')).length;
    ...
    <DashboardLayout>
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-sm md:text-base text-muted-foreground">Comprehensive business insights and metrics</p>
        </div>
